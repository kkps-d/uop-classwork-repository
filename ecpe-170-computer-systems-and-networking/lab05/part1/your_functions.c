...
{
    struct BTreeNode *currNode = *node;
    ...
    while (1)
    {
        if (element < currNode->element)
        {
            ...
            {
                currNode->leftnode = calloc(4, sizeof currNode->leftnode);
                currNode->leftnode->element = element;
                return;
            }
            else
            {
        ...
        else
        {
            ...
            {
                currNode->rightnode = calloc(4, sizeof currNode->rightnode);
                currNode->rightnode->element = element;
                return;
            }
            else
            {
...
{
    free_btree_subnodes(*node);
}

void free_btree_subnodes(struct BTreeNode *node) {
    if (node != NULL) {
        free_btree_subnodes(node->leftnode);
        free(node->leftnode);
        free_btree_subnodes(node->rightnode);
        free(node->rightnode);
    }
}
...
void tree_sort(int *array, int size)
{
    ...
    struct BTreeNode *node = calloc(4, sizeof *node);
    node->element = array[0];

    for (int i = 1; i < size; i++)
    {
        insert_element(&node, array[i]);
    }
    int i = 0;
